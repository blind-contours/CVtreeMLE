...
row that has $X_2*X_5$. The Mixture ATE shows the ATE, or the expected mean 
...
```
Above we see the fold specific results for $X_2 * X_5$. Let's focus on $X_2$ in 
the rules listed under _mix_rule_. Here we see that in fold 1 the threshold
for $X_2$ is 1.046, in fold 2 it's 1.057 etc. Of these thresholds we select
the lowest because this value covers observations indicated by higher 
thresholds. So in our union rule the threshold for $X_2$ is 0.727 because 
...
the union rule is X1 > 0.727 & X1 < 2.467 & X5 > 0.067 & X5 < 2.526. Overall, 
the ATE for this rule is 6.5 for the pooled TMLE estimate. We see this is
higher than if we were to simply take a weighted average.
...
As we can see, $X_2 * X_5$ is listed as having competitive antagonism. This is 
