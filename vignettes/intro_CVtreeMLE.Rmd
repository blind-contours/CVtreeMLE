...
ATE is 7.552 which means the endocrine disruption outcome is 7.552 higher
...
On the Y-axis, Fold 6 is the overall pooled estimate. This is a weighted average
...
fold_marginal_results %>%
  ...
  kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria")
```
This gives a better picture of what's going on. Above we see the fold specific
ATEs for comparing the region above the reference region for $X_1$. We can see 
that the reference values and region above the reference changes across the 
folds. This is to be expected given using 5-fold CV. When using 10-fold CV,
based on these estimates we'd expect the threshold to be around 2.077 since
this is found in a majority of the folds. The v-fold specific results come
with valid inference

```{r plot sim_mixture_results, fig.height = 4, fig.width = 12}
...
`CVtreeMLE` will give, in the even that a signal actually exists, more 
