...
This algorithm is applied to $A$ as a mixture to generate a decision tree that includes multiple different exposures in $A$ while adusting for $W$ non-parametrically. Likewise, the backfitting algorithm is applied to each individual component of $A$, $A_i$, while controlling for other mixture variables $A_{\ne i}$ and $W$. In this way, univariate thresholds are determined for each mixture variable individually. 
