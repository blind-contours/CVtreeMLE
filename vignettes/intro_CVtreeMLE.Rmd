...
estimating the effects of a data adaptively. There is a literature on the 
dangers of deriving parameters data-adaptively and the common
...
to the rule compared to if no one was exposed to the mixture. For a technical 
...
results that are found consistenly across all the folds:
...
row that has $X_1*X_5$. The Mixture ATE shows the ATE, or the expected mean 
...
vfold_mixture_results$X1X5 %>%
  kableExtra::kbl(caption = "X1 and X5 Interaction") %>%
  kableExtra::kable_classic(full_width = F, html_font = "Cambria")

```
Above we see the fold specific results for $X_1 * X_5$. Let's focus on $X_1$ in 
the rules listed under _mix_rule_. Here we see that in fold 1 the threshold
for $X_1$ is 0.998, in fold 2 it's 0.992 etc. Of these thresholds we select
the lowest because this value covers observations indicated by higher 
thresholds. So in our union rule the threshold for $X_1$ is 0.992 because 
this threshold covers all other observations indicated by slightly higher
rules in the other folds. The same is true for $X_5$ but we select the 
highest threshold because the direction is $<=$. This is why in the first table
the union rule is X1 > 0.992 & X1 < 4.687 & X5 > 0.067 & X5 < 2.521. Overall, 
the ATE for this rule is 8.060. What happens is that the nuisance parameters 
...
![NIEHS_fig](NIEHS_interactions.png|width=100px)


As we can see, $X_1 * X_5$ is listed as having competitive antagonism. This is 
picked up in `CVtreeMLE` because the thresholds that define the ATE are for 
regions where $X_1$ is high and $X_5$ is low. Thus the impact is greater in 
this region because $X_5$ antagonizes the effect of $X_1$. The ground-truth data
also has interactions for $X_1 * X_2$ which we detect amongst others. Generally,
