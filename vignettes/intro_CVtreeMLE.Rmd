...
to if no individuals were exposed to the mixture rule. So in the first row the
ATE is 7.5 which means the endocrine disruption outcome is 7.5 higher
...
We see this is higher than if we were to simply take a weighted average which
is shown in the pooled row above. 
...
pooled_mixture_results %>%
  ...
  kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria")
```
So here we see that, when using 5-fold CV, we detect 4 of thre 6 built in 
interactions. 
...
Above we see the ATE estimates for thresholds determined in $X_7*X_5$. Overlaid
...
The ATE for this region was 7.5 meaning that, the outcome was 7.5 higher
...
95% CI between 6.6-8.4. The region that includes all the individuals used
in this ATE estimate was: X5 > 0.05 & X5 < 1.324 & X7 > 0.242 & X7 < 4.886. 

There was some fluctuation of the fold specific thresholds used in this 
mixture rule. For X5 threshold values _1.332_, 0.374, 0.611, 0.313, and 0.709.
_1.332_ was used in the union rule because it encompasses all the observations
indicated from the other rules. For X7, threshold values 0.669, 0.958, 0.503,
0.611, and 0.24 were found and therefore _0.24_ was used as the lower bound 
which encompasses all these observations. 

Overall our results show that individuals who are exposed to levels of X5 at 
levels less than 1.332 and X7 at levels greater than 0.24 have outcomes that 
are 7.5 higher compared to those who are not exposed to this combination 
of exposures. 
...
region for that exposure. So in fold 1 that region is X1 < 1.762 & X1 < 0.998 &
X1 < 0.329. Or more simply X1 < 0.329. In fold 2 this reference region is X1 <
0.5, fold 3 X1 < 0.48, fold 4 X1 < 0.3 and fold 5 X1 < 2.07. So fold 5 induces
some instability as the rest of the reference regions were defined pretty 
consistently. This can be improved by using more folds as we recommend. We 
compare each region above the reference region to the reference region when
...
is 1.421 (-.003 - 2.846). Node 6 compared to node 4 has an ATE of 5.89 (4.854 - 
6.933) etc. Overall `CVtreeMLE` gives valid inference for the terminal nodes
of the best fitting decision tree. 

It should be noted that the higher regions above the reference region are not 
found across all the folds. Let's look again at our output for X1. 

```{r X1_higher_thresholds_reduce_example}
fold_marginal_results %>%
  filter(var == "X1") %>%
  kableExtra::kbl(
  ...
  kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria")
```
Here we see that for comparing X1 at a region directly above the reference to
the reference (X1_2 - X1_1) we have 5 results plus a pooled estimate (weighted
average with a union rule). Because only 4 folds included a tree that had 
two levels above the reference (X1_3 - X1_1) we have only 4 results. This 
is the same for a trees with four terminal nodes. At five terminal nodes we 
see only 2 folds included these trees and likewise for six nodes only one. 
We can revist the plots above for this X1 variable to see this variability. 

As such, when reporting v-fold specific results, it is important to include 
these consistency measures. Again, it is likely these trees will all be more
similar, and therefore results will be cleaner and more interpretable when
higher folds are used. 
...
The marginal rule is what we compare to the reference level. 
Reference rules can be found 
here: 
