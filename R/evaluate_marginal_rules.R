...
#' @param no_marg_rules TRUE/FALSE if no marginal rules were found across the efolds
...
evaluate_marginal_rules <- function(data, marg_decisions, no_marg_rules, mix_comps) {
  marg_additive_data <- list()

  if (no_marg_rules == FALSE) {
    for (i in seq(nrow(marg_decisions))) {
      target_marg_rule <- marg_decisions$rules[i]
      rule_name <- base::paste(marg_decisions$target_m[i], "marg_rule", marg_decisions$quantile[i], sep = "_")
