...
format_RMSE_for_models <- function(marginal_results, mixture_results, additive_RMSE_star, marg_combo_data){
  ...
  marginal_rules_RMSE <- cbind(marginal_results$`Marginal Rules`, marginal_results$RMSE)
  mixture_rules_RMSE <- cbind(mixture_results$`Mixture Interaction Rules`, mixture_results$RMSE)
  additive_RMSE <- cbind("additive marginal model", additive_RMSE_star)
  ...
  non_additive_RMSE <- cbind("non-additive marginal model", non_additive_RMSE )

  models_RMSE <- as.data.frame(rbind(marginal_rules_RMSE, mixture_rules_RMSE, additive_RMSE, non_additive_RMSE))

  colnames(models_RMSE) <- c("Model", "RMSE")
