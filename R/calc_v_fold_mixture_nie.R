#' @title Calculate the NIE for V-fold specific rules
#'
...
#' for each rule do a TMLE update step to target the natural indirect effect.
...
calc_v_fold_mixtures_nie <- function(input_mix_rules,
                                     input_mix_data,
                                     y,
                                     n_folds) {
  ...
  for (fold in seq(input_mix_data)) {
    ...
    for (rule in seq(fold_data)) {
      ...
      sqrd_resids <- (mix_data$qbar_azw - mix_data[y])^2
